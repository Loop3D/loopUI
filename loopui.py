...
def dist_wavelet2D(img1,img2,n_levels,n_bins,plot=False,verb=False):
    ...
    for l in range(n_levels):
        ...
        pA1,_ = np.histogram(cA1,bins=binedges)
        pA1 = pA1/np.prod(cA1.shape)
        pA2,_ = np.histogram(cA2,bins=binedges)
        pA2 = pA2/np.prod(cA2.shape)
        tmp_min = np.min([np.amin(cH1),np.amin(cH2)])
        tmp_max = np.max([np.amax(cH1),np.amax(cH2)])
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        pH1,_ = np.histogram(cH1,bins=binedges)
        pH1 = pH1/np.prod(cH1.shape)
        pH2,_ = np.histogram(cH2,bins=binedges)
        pH2 = pH2/np.prod(cH2.shape)
        tmp_min = np.min([np.amin(cV1),np.amin(cV2)])
        tmp_max = np.max([np.amax(cV1),np.amax(cV2)])
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        pV1,_ = np.histogram(cV1,bins=binedges)
        pV1 = pV1/np.prod(cV1.shape)
        pV2,_ = np.histogram(cV2,bins=binedges)
        pV2 = pV2/np.prod(cV2.shape)
        tmp_min = np.min([np.amin(cD1),np.amin(cD2)])
        tmp_max = np.max([np.amax(cD1),np.amax(cD2)])
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        pD1,_ = np.histogram(cD1,bins=binedges)
        pD1 = pD1/np.prod(cD1.shape)
        pD2,_ = np.histogram(cD2,bins=binedges)
        pD2= pD2/np.prod(cD2.shape)
...
def dist_wavelet3D(img1,img2,n_levels,n_bins,plot=False,verb=False):
    ...
    for l in range(n_levels):
        ...
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        paad1,_ = np.histogram(caad1,bins=binedges)
        paad1 = paad1/np.prod(caad1.shape)
        paad2,_ = np.histogram(caad2,bins=binedges)
        paad2 = paad2/np.prod(caad2.shape)
        tmp_min = np.min([np.amin(cada1),np.amin(cada2)])
        tmp_max = np.max([np.amax(cada1),np.amax(cada2)])
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        pada1,_ = np.histogram(cada1,bins=binedges)
        pada1 = pada1/np.prod(cada1.shape)
        pada2,_ = np.histogram(cada2,bins=binedges)
        pada2 = pada2/np.prod(cada2.shape)
        tmp_min = np.min([np.amin(cdaa1),np.amin(cdaa2)])
        tmp_max = np.max([np.amax(cdaa1),np.amax(cdaa2)])
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        pdaa1,_ = np.histogram(cdaa1,bins=binedges)
        pdaa1 = pdaa1/np.prod(cdaa1.shape)
        pdaa2,_ = np.histogram(cdaa2,bins=binedges)
        pdaa2 = pdaa2/np.prod(cdaa2.shape)
        tmp_min = np.min([np.amin(cadd1),np.amin(cadd2)])
        tmp_max = np.max([np.amax(cadd1),np.amax(cadd2)])
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        padd1,_ = np.histogram(cadd1,bins=binedges)
        padd1 = padd1/np.prod(cadd1.shape)
        padd2,_ = np.histogram(cadd2,bins=binedges)
        padd2 = padd2/np.prod(cadd2.shape)
        tmp_min = np.min([np.amin(cdad1),np.amin(cdad2)])
        tmp_max = np.max([np.amax(cdad1),np.amax(cdad2)])
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        pdad1,_ = np.histogram(cdad1,bins=binedges)
        pdad1 = pdad1/np.prod(cdad1.shape)
        pdad2,_ = np.histogram(cdad2,bins=binedges)
        pdad2 = pdad2/np.prod(cdad2.shape)
        tmp_min = np.min([np.amin(cdda1),np.amin(cdda2)])
        tmp_max = np.max([np.amax(cdda1),np.amax(cdda2)])
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        pdda1,_ = np.histogram(cdda1,bins=binedges)
        pdda1 = pdda1/np.prod(cdda1.shape)
        pdda2,_ = np.histogram(cdda2,bins=binedges)
        pdda2 = pdda2/np.prod(cdda2.shape)
        tmp_min = np.min([np.amin(cddd1),np.amin(cddd2)])
        tmp_max = np.max([np.amax(cddd1),np.amax(cddd2)])
        binedges = np.linspace(tmp_min,tmp_max,num=int(n_bins+1))
        pddd1,_ = np.histogram(cddd1,bins=binedges)
        pddd1 = pddd1/np.prod(cddd1.shape)
        pddd2,_ = np.histogram(cddd2,bins=binedges)
        pddd2 = pddd2/np.prod(cddd2.shape)
